...
import { binaryExpression, isBinaryExpression, isLogicalExpression, logicalExpression } from '@babel/types'

const binaryVisitor = {
  BinaryExpression(path: any) {
    const node = path.node
    const operatorList = ['<', '>', '<=', '>=']
    if (isBinaryExpression(node.left) && operatorList.includes(node.operator)) {
